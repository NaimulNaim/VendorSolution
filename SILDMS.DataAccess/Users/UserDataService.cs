    ...
    public class UserDataService : IUserDataService
    {
        ...
        public string AddUser(SEC_User objUser, string action, out string errorNumber)
        {
            ...
            try
            {
                DatabaseProviderFactory factory = new DatabaseProviderFactory();
                SqlDatabase db = factory.CreateDefault() as SqlDatabase;
                using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("SetVendorUsers"))
                {
                    // Set parameters 
        ...
        public bool IsValidUser(string user, string password, string ip, out List<GetUserAccessPermission_Result> accessList)
        {
            ...
            DatabaseProviderFactory factory = new DatabaseProviderFactory();
            SqlDatabase db = factory.CreateDefault() as SqlDatabase;
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("VCMS_CheckUserValidityForVendor"))
            {
                // Set parameters 
